...
<template>
  <div class="CustomizeHomeFeatures -mt-5 mb-20">
    <template v-for="key in Object.keys(notes)" :key="key">
      <h4 class="font-bold text-white mb-1 mt-5">
        {{ key }}
      </h4>
      <div class="notes">
        <router-link data-aos="flip-up" v-for="(item, index) in notes[key]" :key="index" :to="item.link" class="column">
          <div class="ct">
            <h3 class="text-normal">{{ item.name }}</h3>
